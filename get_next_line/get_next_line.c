...
int spit_line(char **back, int checkline, char **line)
{
	char *point;
	(*back)[checkline] = '\0';
	*line = ft_strdup(*back);
...
int					return_all(char **backup, char **line, int read_size)
{
	...
	if (read_size < 0)
		return (-1);
	if (*backup && (cut_idx = ft_checkline(*backup)) >= 0)
		return (spit_line(backup, cut_idx, line));
	else if (*backup)
	{
		*line = *backup;
		*backup = 0;
...
int get_next_line(int fd, char **line)
{
	...
	while((sizeofread = read(fd, buffer, BUFFER_SIZE)) > 0)
	{
		...
		if ((checkline = ft_checkline(back[fd])) >= 0)
			return (spit_line(&back[fd], checkline, line));
	}
